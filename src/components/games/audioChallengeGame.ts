const renderAudioChallengeGameTemplate = () => `
<h1 class="game-title">Аудиовызов</h1>
<div class="game-operations-group"></div>
<section class="game-section"> 
    <div class="game-section__microphone-wrapper"></div>
    <div class="game-section__answer-wrapper"></div>
    <div class="game-section__words-wrapper"></div>
    <div class="game-section__btn-wrapper"></div>
</section>
`;

export default renderAudioChallengeGameTemplate;